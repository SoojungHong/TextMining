...
def get_vec_by_pre_trained_doc2vec(doc2vec_model, input_str):
    tokens = input_str.split()
    doc_vec = doc2vec_model.infer_vector(tokens)
    return doc_vec


def get_desc_vec_by_pre_trained_doc2vec(model, input_str):
    tokens = input_str.split()
...
def reduce_dimension_PCA(df):
    pca = PCA(n_components=4)  # number of components to keep
    description_list = (df['desc_vec'].values).tolist()
    query_vec = get_vec_by_pre_trained_doc2vec(m, df['query_raw'].iloc[0])
    query_name = df['query_raw'].iloc[0]
    description_list.append(query_vec)  # add the vector of the query itself as origin
    result_df = pca.fit_transform(description_list)
    return result_df


def run_gaussian_mixture_model_and_plot(df):
    gmm = GaussianMixture(n_components=2, n_init=10).fit(df)  # say, two clusters
    ax = None  #ToDo : why I need this? Do clean up if it is not necessary
    label = True
    ax = ax or plt.gca()
    ...
    # plot the data point in the 3 dimensional space
    ax.scatter(df['x'], df['y'], df['z'], c=labels, cmap='viridis', edgecolor='k', s=40, alpha=0.5)

    # assign labels into points - place name, query(origin) name)
    places = list(final_df['place_desc'].values)
    result_index = list(final_df['result_index'].values)
    ...
    for i in range(len(result)):
        if i == (len(result) - 1):
            ...
            point_labels.append(place_names[i] + ':' + places[i] + ':' + str(result_index[i]))

    # add the text label for each data points next to data point
...
def run_kmeans_clustering(df, final_df):
    ...
    for i in range(len(result)):
        if i == (len(result) - 1):
            ...
            point_labels.append(place_names[i] + ':' + str(result_index[i]))  # + ':' + places[i])

    for x_label, y_label, z_label, label in zip(df['x'], df['y'], df['z'], point_labels):
        ax.text(x_label + 0.02, y_label + 0.02, z_label + 0.01, label, size=6)

    for idx, centroid in enumerate(centroids):
        ax.scatter(*centroid, cmap=colmap[idx + 1])
        ax.text(centroid[0], centroid[1], centroid[2] + 0.1, 'cluster_{}'.format(idx), None, size=6, color='Red')

    pyplot.show()


# data
RECO_HEADER = [#somedata]
RECO_DATA = "C:hahaha.csv"

# read data
df = read_file(RECO_DATA, RECO_HEADER)  # (209921, 18)

# get records contain given the input query
ret_query = get_long_queries_contains("'mexican restaurants near my location'", df)  # "'african restaurant near me'", df)
...
# for places are not related with restaurant such as gas station
#one_query_records.to_csv('C:/Users/shong/Documents/data/one_query_records.csv')
...
# add 'desc_vec' : vector of place description
final_df['desc_vec'] = final_df.apply(lambda row: get_vec_by_pre_trained_doc2vec(m, row.place_desc), axis=1)

# recommendation data frame
labels = ['query_raw', 'name_chosen', 'place_desc', 'desc_vec', 'distance']
result = reduce_dimension_PCA(final_df)

# write the final experiment dataframe to csv file
#final_df.to_csv('C:/Users/shong/Documents/data/final_df_mexican_near_restaurant.csv')

# prepare z-axis value (distance value), normalized z-value
...
run_gaussian_mixture_model_and_plot(df)

# clustering with K-means algorithm
run_kmeans_clustering(df, final_df)
